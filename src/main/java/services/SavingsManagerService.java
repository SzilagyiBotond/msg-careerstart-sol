...
public class SavingsManagerService {
    ...
    public void passTime() {
        ...
        savingAccounts.forEach(savingAccount -> {
            if (savingAccount.getInterestFrequency() == CapitalizationFrequency.MONTHLY) {
                ...
                addQuarterly(savingAccount,nextSystemDate);
            }
        });
    ...
    private void addQuarterly(SavingsAccountModel savingsAccount,LocalDate currentInterestMonth){
        LocalDate nextInterestDateForAccount = savingsAccount.getLastInterestAppliedDate().plusMonths(3);

        if (isSameMonthAndYear(currentInterestMonth,nextInterestDateForAccount)){
            addInterest(savingsAccount);
            savingsAccount.setLastInterestAppliedDate(currentInterestMonth);
        }
    }
